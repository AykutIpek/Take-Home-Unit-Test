...
class JSONMapperTests: XCTestCase {
    ...
    func test_with_valid_json_successfully_decodes() {
        ...
        XCTAssertNoThrow(try StaticJSONMapper.decode(file: "UserStaticData", type: UserResponse.self), "Mapper shouldn't throw an error")
        
        let userResponse = try? StaticJSONMapper.decode(file: "UserStaticData", type: UserResponse.self)
    ...
    func test_with_missing_file_error_thrown() {
        XCTAssertThrowsError(try StaticJSONMapper.decode(file: "", type: UserResponse.self), "An error should be thrown")
        do {
            _ = try StaticJSONMapper.decode(file: "", type: UserResponse.self)
        } catch {
    ...
    func test_with_invalid_file_error_thrown() {
        XCTAssertThrowsError(try StaticJSONMapper.decode(file: "dfddf", type: UserResponse.self), "An error should be thrown")
        do {
            _ = try StaticJSONMapper.decode(file: "dfdfdfd", type: UserResponse.self)
        } catch {
            guard let mappingError = error as? StaticJSONMapper.MappingError else {
                XCTFail("This is the wrong type of error for missing files")
                return
    ...
    func test_with_invalid_json_error_thrown() {
        ...
        XCTAssertThrowsError(try StaticJSONMapper.decode(file: "UserStaticData", type: UserDetailResponse.self), "An error should be thrown")
